...
function AlertsCtrl(e){e.alerts=[{type:"success",msg:"type: success"},{type:"danger",msg:"type: warning"}],e.addAlert=function(){e.alerts.push({msg:"Another alert!"})},e.closeAlert=function(t){e.alerts.splice(t,1)}}angular.module("RDash").controller("AlertsCtrl",["$scope",AlertsCtrl]);
function ContainersCtrl(t){t.containers=[{id:"51b",image:"dev-front:latest",command:"npm start",created:"27 hours ago",status:"Up 27 hours",port:"0.0.0.0:80->8080/tcp",name:"dev-front"},{id:"527",image:"dev-api:latest",command:"supervisord -c supervisord.conf",created:"27 hours ago",status:"Up 27 hours",port:"0.0.0.0:443->8080/tcp",name:"dev-api"}]}angular.module("RDash").controller("ContainersCtrl",["$scope",ContainersCtrl]);
function ImagesCtrl(e){e.images=[{repo:"pre-front",tag:"latest",imageID:"ef8",created:"27 hours ago",size:"521.3 MB"},{repo:"pre-api",tag:"latest",imageID:"d7c",created:"27 hours ago",size:"542.2 MB"}]}angular.module("RDash").controller("ImagesCtrl",["$scope",ImagesCtrl]);
...
function rdContainer(){var t={restrict:"AE",template:'<td class="text-center">{{ container.id }}</td><td>{{ container.image }}</td><td>{{ container.command }}</td><td>{{ container.status }}</td><td>{{ container.port }}</td><td>{{ container.name }}</td>'};return t}angular.module("RDash").directive("rdContainer",rdContainer);

...
"use strict";angular.module("RDash").config(["$stateProvider","$urlRouterProvider",function(t,e){e.otherwise("/"),t.state("index",{url:"/",templateUrl:"templates/dashboard.html"}).state("list",{url:"/list",templateUrl:"templates/list.html"}).state("trash",{url:"/trash",templateUrl:"templates/trash.html"})}]);
...
function ContainersCtrl(r){r.containers=[{id:"51b",image:"dev-front:latest",command:"supervisord -c supervisord.conf",created:"27 hours ago",status:"Up 27 hours",port:"0.0.0.0:80->8080/tcp",name:"dev-front"},{id:"527",image:"dev-api:latest",command:"supervisord -c supervisord.conf",created:"27 hours ago",status:"Up 27 hours",port:"0.0.0.0:443->8080/tcp",name:"dev-api"}],r.addAlert=function(){r.alerts.push({msg:"Another alert!"})},r.closeAlert=function(e){r.alerts.splice(e,1)}}angular.module("RDash").controller("ContainersCtrl",["$scope",ContainersCtrl]);
